...
uint8_t ctaphid_handle_packet(uint8_t * pkt_raw)
{
    ...
    printf1(TAG_GREEN, "checking types here: %x\n", cmd);
    switch(cmd)
    {
        ...
        case CTAPHID_MSG:
            ...
            printf1(TAG_GREEN, "in msg!!!\n");
            printf1(TAG_HID,"CTAPHID_MSG\n");
            if (len == 0)
            {
                printf1(TAG_GREEN, "len error\n");
            ...
            printf1(TAG_GREEN, "made it here\n");
            is_busy = 1;
            ctap_response_init(&ctap_resp);
        ...
        default:
            printf1(TAG_GREEN, "in default\n");
            if (ctaphid_custom_command(len, &ctap_resp, &wb) != 0){
                printf1(TAG_GREEN, "custom command!\n");
                is_busy = 0;
            }else{
